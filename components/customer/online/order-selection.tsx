...
export default function OrderSelection({ onSelectOrder }: OrderSelectionProps) {
    ...
    const fetchPendingOrders = useCallback(async () => {
        ...
        try {
            // TODO: BACKEND - CRITICAL SECURITY STEP
            // We need to get the current logged-in user so we ONLY fetch THEIR orders.
            ...
            // TODO: BACKEND - Make sure to add the correct foreign key filter (e.g., .eq("customer_id", user.id))
            // I've added a placeholder .eq() below. Adjust "customer_id" to match your actual DB column.
            ...
            console.error(e);
            setError("Failed to fetch your active orders.");
        } finally {
    ...
    const filteredOrders = pendingOrders.filter((order) => {
        ...
        // Customers usually just search by ID since the name is always theirs, but keeping name search just in case
        const name = (order.name || "").toLowerCase();
        return shortId.includes(query) || name.includes(query);
    });

    return (
        <div className="space-y-4 animate-in slide-in-from-left-4 duration-300 w-full">
            {error && (
                    ...
                    filteredOrders.map((order) => {
                        ...
                        return (
                            <button
                                ...
                                className="w-full bg-[#f8fbfd] hover:bg-[#e8eef1] border-2 border-[#1e3d58]/10 hover:border-[#43b0f1] transition-all rounded-[20px] p-4 text-left group shadow-sm"
                            >
                                ...
                                <div className="flex items-center gap-2 mb-1">
                                    ...
                                    <span className="text-sm font-bold text-gray-700 truncate">{order.name}</span>
                                </div>
                                <div className="flex items-center gap-2">
