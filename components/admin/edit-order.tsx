...
export default function EditOrderForm() {
    ...
    const handleSearch = async () => {
        if (!orderId.trim()) {
            setErrors({ orderId: "Please enter an Order ID to search." });
            return;
        }
        ...
        const cleanOrderId = orderId.trim().replace(/^ORD-/, '');
        ...
        try {
            ...
            if (error || !data) {
                setGlobalError("Order not found. Please check the ID.");
                return;
            }

            if (data.current_status !== "Pending") {
                setGlobalError(`Cannot edit this order. Current status is: ${data.current_status}. Only "Pending" orders can be edited.`);
                return;
            }

            setCustomerName(data.name || "N/A");
            setContactNumber(data.number || "N/A");
            setCustomerZone(data.location_pricing?.location_name || "N/A");
            setPricePerUnit(data.location_pricing?.location_price || 0);
    ...
    const validateForm = () => {
        ...
        if (!hasLoadedOrder) {
            newErrors.orderId = "Please search for a valid pending order first.";
        }
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const slimProduct = allProducts?.find((p: any) => p.product_name?.toLowerCase().includes("slim"));
            const roundProduct = allProducts?.find((p: any) => p.product_name?.toLowerCase().includes("round"));
            ...
            const saveItem = async (productId: any, quantity: number) => {
                if (!productId) return; 
                    ...
                    <div className="mb-6">
                        ...
                        <div className="relative">
                            <input
                                ...
                                className={`w-full h-14 px-6 rounded-full border-2 bg-white text-[#1e3d58] font-bold text-lg focus:outline-none focus:ring-2 focus:ring-[#43b0f1] disabled:opacity-50 ${errors.orderId ? 'border-red-500' : 'border-[#1e3d58]'}`}
                            />
                            <button 
                                onClick={handleSearch}
                                disabled={loading}
                                className="absolute right-2 top-2 bottom-2 bg-[#1e3d58] text-white px-4 rounded-full hover:bg-[#43b0f1] transition-colors flex items-center justify-center disabled:opacity-50"
                            >
                               {loading && !hasLoadedOrder ? <span className="animate-spin border-2 border-white border-t-transparent rounded-full w-5 h-5"></span> : <Search size={20} />}
                            </button>
                        </div>
                        {errors.orderId && <p className="text-red-500 text-sm font-bold mt-1 ml-2">{errors.orderId}</p>}
                    </div>

                    <div className={`space-y-5 transition-opacity ${!hasLoadedOrder ? 'opacity-50 pointer-events-none' : 'opacity-100'}`}>
                        ...
                        <div className="grid grid-cols-1 gap-4 opacity-80">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                 <div>
                                    ...
                                    <div className="flex h-12 items-center px-4 rounded-full border-2 border-gray-300 bg-gray-50 text-gray-600 font-bold overflow-hidden text-ellipsis whitespace-nowrap">
                                        <MapPin size={16} className="mr-2 flex-shrink-0"/> {customerZone}
                                    </div>
                                </div>
                                <div>
                        ...
                        <div className="pt-4 flex justify-center">
                            <Button onClick={handleConfirmClick} disabled={loading || !hasLoadedOrder} className="w-full h-14 text-2xl font-bold rounded-full bg-[#43b0f1] text-white border-2 border-[#43b0f1] hover:bg-[#1e3d58] hover:border-[#1e3d58] transition-all active:scale-95 disabled:opacity-50 shadow-md">
                                {loading && hasLoadedOrder ? "Saving..." : "Confirm Edit"}
                            </Button>
                        </div>
                    </div>
                </div>
