  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50 font-sans selection:bg-[#43b0f1]/20 overflow-x-hidden">
      ...
      <main className="flex-1 flex flex-col w-full">
        <section className="relative bg-gradient-to-b from-[#43b0f1] to-[#2c88d1] text-white pt-20 pb-32 px-4 sm:px-6 lg:pt-32 lg:pb-48 overflow-hidden w-full">
          <div className="absolute top-0 left-0 w-full h-full overflow-hidden opacity-10 pointer-events-none">
            <Droplets className="absolute top-10 right-10 w-64 h-64 text-white animate-pulse shrink-0" />
            <div className="absolute bottom-0 left-[-20%] w-[50%] h-[50%] bg-white rounded-full blur-[100px]" />
          </div>

          <div className="max-w-4xl mx-auto text-center relative z-10 space-y-6 w-full">
            <h5 className="text-sky-100 font-bold tracking-widest uppercase text-sm lg:text-base mb-4 break-words whitespace-normal leading-tight">
              RFinest Water Refilling Station
            </h5>
            <h1 className="text-5xl lg:text-7xl font-black tracking-tight leading-tight drop-shadow-sm break-words whitespace-normal w-full">
              Skip the Text. <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-white to-sky-200 italic">
                Just Tap.
              </span>
            </h1>
            <p className="text-lg lg:text-2xl font-medium text-sky-50 max-w-2xl mx-auto leading-relaxed opacity-95 break-words whitespace-normal w-full">
        ...
        <section className="bg-[#1e3d58] text-white py-16 px-4 sm:px-6 lg:py-24 relative z-20 w-full">
          <div className="max-w-5xl mx-auto w-full">
            <div className="text-center mb-16 space-y-4 w-full">
              <h2 className="text-3xl lg:text-5xl font-black uppercase tracking-tight break-words whitespace-normal leading-tight w-full">
                Why Switch to Digital?
              </h2>
              <p className="text-sky-200 text-lg lg:text-xl font-medium italic break-words whitespace-normal w-full">
        ...
        <section className="bg-white py-20 px-4 sm:px-6 lg:py-32 text-center w-full">
          <div className="max-w-3xl mx-auto space-y-10 w-full">
            <h3 className="text-4xl lg:text-6xl font-black text-[#1e3d58] leading-tight tracking-tighter break-words whitespace-normal w-full">
            ...
            <p className="text-slate-400 font-bold italic text-sm tracking-widest uppercase pt-8 break-words whitespace-normal w-full">
              John 4:14 "But whosoever drinks the water I give them will never thirst."
            </p>
          </div>
        </section>
      </main>
...
function BenefitCard({ title, description }: { title: string; description: string }) {
  return (
    <div className="flex flex-col items-center text-center lg:items-start lg:text-left p-6 rounded-[30px] bg-[#264a6a] border border-[#43b0f1]/20 shadow-lg transition-transform hover:-translate-y-1 w-full min-w-0">
      <div className="mb-4 p-3 bg-[#43b0f1]/20 rounded-full text-[#43b0f1] shrink-0">
        <CheckCircle2 size={32} strokeWidth={3} />
      </div>
      <h4 className="text-2xl font-black mb-3 tracking-tight break-words whitespace-normal leading-tight w-full">{title}</h4>
      <p className="text-sky-100 leading-relaxed font-medium opacity-90 break-words whitespace-normal w-full">
        {description}
      </p>
    </div>
  );
